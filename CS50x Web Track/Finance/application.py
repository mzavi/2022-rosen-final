...
def buy():
    ...
    if request.method == "GET":
        ...
        if stock == None:
            return apology(symbol, "Symbol does not exist")

        if shares < 0:
            return apology("Invalid Shares: Pick a Positive Number")
        ...
        db.execute("INSERT INTO transactions (user_id, transaction_type, symbol, shares, price, date) VALUES(?,'Buy', ?, ?, ?, ?)", user_id, stock["symbol"], shares, stock["price"], date)
...
def quote():
    ...
    if request.method == "GET":
        ...
        if stock == None:
            return apology(symbol, "Symbol does not exist")
            #Maybe doesn't work on Sundays becaue stock exchange is CLOSED!??
...
def register():
    ...
    if request.method == "GET":
        ...
        if not password:
            return apology("Must give passsord")
...
def sell():
    ...
    if request.method == "GET":
        ...
        db.execute("INSERT INTO transactions (user_id, transaction_type, symbol, shares, price, date) VALUES(?,'Sell', ?, ?, ?, ?)", user_id, stock["symbol"], (-1)*shares, stock["price"], date)
